...
class lining_furnacebottom extends StatefulWidget {
  final DateTime timestamp;

  const lining_furnacebottom({
    Key? key,
    required this.timestamp,
  }) : super(key: key);

  @override
  State<lining_furnacebottom> createState() => _SinteringChecklistState();
}

class _SinteringChecklistState extends State<lining_furnacebottom> {
  ...
  void initState() {
    super.initState();
    getUser();
    _sinteringItems = _apiService.fetchListItem('22');
  ...
  Future<void> _postData(List<ListItem> items, String repairId,
    ...
    Map<String, dynamic> jsonData = {
      ...
      "TypeDetail": "ข้อสังเกตุบริเวณก้อนเตา",
      "FurnaceNo": 2,
      "HFT": "2 T",
    ...
    _groupedControllers.forEach((groupName, controllers) {
      ...
      for (int i = 0; i < controllers.length; i++) {
        ...
        // Check if the item has a "Good"/"No Good" button
        if (groupItems[i].typeValue == 'b') {
          // Store the button value in "Value" and the text field in "valuedetail"
          ...
          jsonData['Details'].add({
            ...
            "ValueBit": 1,
            "Value":
                buttonValue.toString(), // Store button value (Good/No Good)
            "valuedetail": enteredText // Store text field input in valuedetail
          });
        } else if (groupItems[i].typeValue == 'v') {
          // If the keyboardType is TextInputType.number, store in "Value"
          ...
          if (intValue != null) {
            jsonData['Details'].add({
              ...
              "TypeNo": groupItems[i].typeNo,
              "ValueBit": 1,
              "Value": intValue.toString(), // Store as "Value" for numbers
              "valuedetail": '' // No string value for numbers
            });
          } else {
            jsonData['Details'].add({
              "CheckID": 0,
              ...
              "TypeNo": groupItems[i].typeNo,
              "ValueBit": 1,
              "Value": '', // No integer value
              "valuedetail": enteredText // Store as "valuedetail" for text
            });
          }
        } else if (groupItems[i].typeValue == 's') {
          // If the keyboardType is TextInputType.text, store in "valuedetail"
          jsonData['Details'].add({
            "CheckID": 0,
            "GroupID": groupItems[i].groupId,
            "TypeNo": groupItems[i].typeNo,
            "ValueBit": 1,
            "Value": '', // No integer value
            "valuedetail": enteredText // Store text input in "valuedetail"
          });
        }
      }
    });
